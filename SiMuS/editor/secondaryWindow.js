...
function drawExportPage(){
    ...
    text+="p=(L,c,l)=>{for(let i=0;i<L.length;i++)o(L[i],c,0,l)};o=(l,c,i,p)=>{if(i>=l.length&&p)i=0;if(l.length>i){with(c){with(createOscillator()){let g=createGain(),t=currentTime;connect(g);g.connect(destination);g.gain.value=l[i][0];frequency.value=l[i][1];type=l[i][2];start(t);stop(t+l[i][3]);onended=_=>{playLine(l,c,i+1,p)}}}}};";
    text+='<br>m=[';
    let musicConverted=[[]];
